...
export default observer( function ProfilePhotos({photos}: Props){
    const {profileStore: {isCurrentUser, uploadPhoto, 
        uploading, setMainPhoto, 
        loading, deletePhoto}} = useStore();
    const [addPhotoMode, setAddPhotoMode] = useState(false);
    const [target, setTarget] = useState('');
    const [deltarget, setTarget] = useState('');
    ...
    function handleDelePhoto(photo:Photo, e: SyntheticEvent<HTMLButtonElement>){
        setTarget(e.currentTarget.name);
                                    ...
                                    {isCurrentUser &&
                                        <Button.Group fluid widths={2}>
                                            <Button 
                                                ...
                                                onClick={(e: SyntheticEvent<HTMLButtonElement>) => { handleSetMainPhoto(photo, e)}}
                                                />
                                                <Button icon='trash' color='red'
                                                    loading={target == photo.id && loading}
                                                    disabled={photo.isMain}
                                                    name={photo.id}
                                                    onClick={(e) => handleDelePhoto(photo, e)}
                                                />
                                        </Button.Group>
                                    }
                                </Card>
